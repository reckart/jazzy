	...
	public class LinkController
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (editor.isEditable()) {
				...
				if (pos >= 0 && href != null) {
					...
					SpellCheckedDocument hdoc =
						(SpellCheckedDocument) editor.getDocument();
					...
					try {
						...
						ReplaceListener listener = new ReplaceListener(elem.getStartOffset(),
								elem.getEndOffset() - elem.getStartOffset(), hdoc);
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (editor.isEditable()) {
				...
				if (pos >= 0
					&& (editor.getDocument() instanceof SpellCheckedDocument)) {
					SpellCheckedDocument hdoc =
						(SpellCheckedDocument) editor.getDocument();
					Element elem = hdoc.getCharacterElement(pos);
					if (!doesElementContainLocation(editor,
						elem,
						pos,
						e.getX(),
						e.getY())) {
					...
					if (curElem != elem || curElemImage) {
						...
						if (elem != null) {
							...
							if (a.getAttribute(StyleConstants.NameAttribute)
								== SpellCheckedDocument.ERROR_STYLE) {
		...
		private boolean doesElementContainLocation(
			...
			if (e != null && offset > 0 && e.getStartOffset() == offset) {
				try {
					...
					Shape s1 =
						ui.modelToView(editor, offset, Position.Bias.Forward);
					Rectangle r1 =
						(s1 instanceof Rectangle)
							? (Rectangle) s1
							: s1.getBounds();
					Shape s2 =
						ui.modelToView(
